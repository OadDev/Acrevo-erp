<x-app-layout>
    ...
    <x-card class="max-w-5xl">
        <form method="POST" action="{{ route('work-orders.update', $workOrder) }}" x-data="{
            materials: @js(old('materials', $materials)),
            labour: @js(old('labour', $labour)),
            equipment: @js(old('equipment', $equipment)),
            transport: @js(old('transport', $transport)),
            misc: @js(old('misc', $misc)),
            procedures: @js(old('procedures', $procedures)),
            timeSchedules: @js(old('time_schedules', $timeSchedules)),
            ...
            <p class="mt-6 text-xs text-gray-400">Execution method, site, client, and actual site usage (material inward, attendance, ledger, etc.) are edited from their own tabs.</p>
